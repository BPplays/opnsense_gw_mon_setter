...
use flexi_logger::with_thread as fl_with_thread;
use uom::si::length::kilometer;
use uom::si::time::second;
use uom::si::information::{bit, byte, kilobyte, megabyte, gigabyte};
use uom::si::f64::Information;
use uom::prefix;
use std::path::{Path, PathBuf};
use flexi_logger::{DeferredNow, AdaptiveFormat};
...
struct Cli {
	...
	config: String,
	#[arg(short = 'l', long = "log_dir", default_value = "")]
	log_dir: String,
}
...
fn main() -> Result<()> {
	...
	let app_name = "opnsense_gw_mon_setter";

	let log_size = Information::new::<megabyte>(5.0);
	let logger: flexi_logger::Logger;

	if !cli.log_dir.trim().is_empty() {
		logger = Logger::try_with_str("info")?
			.log_to_file(
				FileSpec::default()
					.directory(Path::new(&cli.log_dir).join(app_name))
					.basename("current")
					.suffix("log"),
			)
			.format_for_files(fl_with_thread)
			.rotate(
				Criterion::Size(log_size.get::<byte>() as u64),
				Naming::Timestamps,
				Cleanup::KeepLogAndCompressedFiles(1, 5),
			)
	} else {
		logger = Logger::try_with_str("info")?.log_to_stdout()
	...
	for (ifname, probes) in cfg.ifaces.into_iter() {
		...
		for probe in probes.into_iter() {
			...
			thread::Builder::new()
				.name(format!("tracer-{}-{}", ifname_for_tracer, probe_for_tracer.name))
				.spawn(move || {
					if let Err(e) = tracer_thread(ifname_for_tracer.clone(), vec![probe_for_tracer.clone()], shared_for_tracer) {
						log::error!(
							"\n    tracer thread error for iface {} probe {}: {:#}",
							ifname_for_tracer,
							probe_for_tracer.name,
							e,
						);
					}
				})
			...
			thread::Builder::new()
				.name(format!("aggregator-{}-{}", ifname_for_aggregator, probe_for_aggregator.name))
				.spawn(move || {
					if let Err(e) = aggregator_thread(ifname_for_aggregator.clone(), vec![probe_for_aggregator.clone()], shared_for_agg) {
						log::error!(
							"\n    aggregator thread error for iface {} probe {}: {:#}",
							ifname_for_aggregator,
							probe_for_aggregator.name,
							e,
						);
					}
				})
			...
			for target in ips_to_try.into_iter() {
				...
				loop {
					...
					match builder.build() {
						Ok(tracer) => {
							...
							let run_res = tracer.run_with(|round: &Round<'_>| {
								...
								let contains_complete = tr.probes.iter().any(|p| {
									// match on a reference so we don't move out of `p`
									matches!(&p.kind, ProbeKind::Complete { ttl, .. } if *ttl > probe.min_ttl)
								});
							...
							match run_res {
								...
								Err(e) => {
									...
									log::error!(
										"\n    run_with returned Err for {} on iface {}: {:#}",
										target,
										ifname,
										e,
									);
						...
						Err(e) => {
							log::error!(
								"\n    failed to build tracer for {} on iface {}: {:#}",
								target,
								ifname,
								e,
							);
